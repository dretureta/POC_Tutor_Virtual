...
def read_conversations(student_id: uuid.UUID, db: Session = Depends(get_db)):
...
def update_conversation_messages(conversation_id: uuid.UUID, messages: List[schemas.Message], db: Session = Depends(get_db)):
